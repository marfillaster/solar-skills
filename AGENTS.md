...
- `solar-analysis/` — Analysis skill (reads CSV, outputs report)
- `soliscloud-export-hourly/` — Data export skill (fetches from SolisCloud API, writes CSV)
...
Both skills must produce/consume the same 16-column CSV format. The canonical column list and sign conventions are defined in `soliscloud-export-hourly/README.md` under "Output Format".
